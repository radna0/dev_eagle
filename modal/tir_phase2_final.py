...
# Modal configuration
app = modal.App("tir-phase2-final")
...
THROUGHPUT_DRAFT = "/data/nvidia/gpt-oss-120b-Eagle3-throughput"
LONG_CONTEXT_DRAFT = "/data/nvidia/gpt-oss-120b-Eagle3-long-context"

# Benchmarking constants
MAX_MODEL_LEN = 32768
GPU_MEMORY_UTILIZATION = 0.9
K_SAMPLES = 4  # Reduced from 8 for faster iteration in final validation, user asked for detailed quality
TEMPERATURE = 1.0
MIN_P = 0.02
MAX_TOKENS = 2048
...
def run_tir_benchmark(
    ...
    # 2. Initialize vLLM Engine directly (as it's in-process here or via API?
    # The user's script used api_server. We'll use LLMEngine for more direct control if possible,
    # but the tool calling logic is easier if we have an API client.
    # Let's use the API server approach but run it in a subprocess.)

    port = 8001 if "Optimized" in config_name else 8000
    ...
    # 4. Run over first 3 questions (for quick turnaround and quality check)
    ...
    log_file = f"/root/logs/tir_{config_name.lower().replace(' ', '_')}.json"
...
def main():
    configs = [
        ("Throughput Spec1", THROUGHPUT_DRAFT, 1, False),
        ("Long Context K3 Baseline", LONG_CONTEXT_DRAFT, 3, False),
        ("Long Context K3 Optimized", LONG_CONTEXT_DRAFT, 3, True),
    ]

    all_results = {}
    for name, path, n_spec, p2 in configs:
        all_results[name] = run_tir_benchmark.remote(name, path, n_spec, p2)

    print("\n" + "=" * 80)
    print("FINAL TIR BENCHMARK SUMMARY")
    print("=" * 80)
    for name, results in all_results.items():
        ...
        print(
            f"{name:30} | Total Wall-Clock: {total_wall:8.2f}s | Avg Tokens/Q: {total_tokens:8.1f}"
        )
    print("=" * 80)
